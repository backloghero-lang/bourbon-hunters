...
function assertMatch(text, expectedId) {
  ...
  for (const token of toks(text)) for (const index of db.token_index?.[token] || []) candidateIndexes.add(index);
  const candidates = candidateIndexes.size ? [...candidateIndexes].map((index) => db.bottles[index]) : db.bottles;
  ...
  return { text, expectedId, candidateCount: candidates.length, score: matches[0].score, elapsedMs: Number(elapsedMs.toFixed(2)), top3: matches.slice(0, 3) };
}
